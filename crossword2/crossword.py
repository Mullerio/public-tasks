...
class TaskFamily:
    ...
    def start(t: Task) -> None:
        TASK_DIR = pathlib.Path(__file__).parent  # <-- folder where crosswords.py lives
        ASSETS_DIR = TASK_DIR / "assets"
        VALID_WORDS_CSV = ASSETS_DIR / "valid_words.csv"
        VERIFIER_TEMPLATE = ASSETS_DIR / "crossword_verifier_template.py"
        
        shutil.copy(VALID_WORDS_CSV, DEFAULT_ENV_PROMPTS["valid_words_filepath"])

        content = replace_placeholders(VERIFIER_TEMPLATE, t)
